    ...
    <div className="space-y-4">
      ...
      {isHr && <Card>
        ...
        <CardContent className="space-y-5">
          ...
          <div className="rounded-lg border bg-muted/30 p-4 text-sm"><p className="font-medium">How the score is calculated</p><p className="mt-1 text-muted-foreground">The system collects department-aware attendance, tasks and deadlines, task AI analysis, DPRs and outputs, manager DPR quality, task comments, approved leave and ATS activity where available. Each KRA metric receives only the evidence sources configured for that metric. AI converts that evidence into 0–100% achievement, multiplies it by the saved metric weight, and sums the contributions. Missing external evidence lowers confidence instead of being invented.</p></div>
          {existingTemplate?.items?.length ? <div><div className="mb-2 flex items-center gap-2 text-sm font-medium"><Save className="h-4 w-4"/> Active metrics for {selectedDesignation?.title}</div><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Metric</TableHead><TableHead>Target</TableHead><TableHead>Weight</TableHead><TableHead>Mode</TableHead><TableHead>Evidence</TableHead><TableHead>Evaluation</TableHead></TableRow></TableHeader><TableBody>{existingTemplate.items.map((i:any)=><TableRow key={i.id}><TableCell><p className="font-medium">{i.name}</p><p className="text-xs text-muted-foreground">{i.description}</p></TableCell><TableCell>{i.targetText ?? (i.targetValue != null ? `${i.targetValue}${i.unit ?? ''}` : '-')}</TableCell><TableCell>{Number(i.weightPercent).toFixed(1)}%</TableCell><TableCell><span className="text-xs font-medium text-emerald-700">Auto · AI</span></TableCell><TableCell><span className="text-xs font-medium">{i.evidenceSource ?? 'HRMS_ACTIVITY'}</span></TableCell><TableCell><span className="text-xs text-muted-foreground">{i.evaluationMethod ?? 'AI evaluates recorded HRMS evidence only.'}</span></TableCell></TableRow>)}</TableBody></Table></div></div> : null}
        </CardContent>
      </Card>}
    </div>
  </AppShell>;
