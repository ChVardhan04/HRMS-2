...
export default function ReportsPage() {
  ...
  const [dailyEmployeeId, setDailyEmployeeId] = useState('ALL');
  const { data: employeeList } = useEmployees('', 100, 1, false);
  const { data: dailyActivity } = useQuery({
    queryKey: ['reports','daily-activity',dailyDate,dailyEmployeeId],
    queryFn: () => api.get<any>(`/reports/daily-activity?date=${dailyDate}${dailyEmployeeId !== 'ALL' ? `&employeeId=${dailyEmployeeId}` : ''}`),
    enabled: isHr,
  });
    ...
    <AppShell title="HR Reports">
      <div className="flex flex-col gap-4">
        ...
        {isHr && (
          <Card>
            <CardHeader>
              <CardTitle>Daily attendance, DPR & To-Do report</CardTitle>
              <p className="text-xs text-muted-foreground">Use this for a specific day. Every active employee is shown, including employees who did not submit a DPR.</p>
            </CardHeader>
            <CardContent>
              <div className="mb-4 flex flex-wrap gap-2">
                <Input type="date" value={dailyDate} onChange={(e)=>setDailyDate(e.target.value)} className="w-44" />
                <select className="h-10 min-w-64 rounded-md border border-input bg-background px-3 text-sm" value={dailyEmployeeId} onChange={(e)=>setDailyEmployeeId(e.target.value)}>
                  <option value="ALL">All employees</option>
                  {(employeeList?.data ?? []).map((e:any)=><option key={e.id} value={e.id}>{e.firstName} {e.lastName} · {e.employeeCode}</option>)}
                </select>
              </div>
              <div className="overflow-x-auto">
                <Table><TableHeader><TableRow><TableHead>Employee</TableHead><TableHead>Attendance</TableHead><TableHead>Check-in</TableHead><TableHead>Check-out</TableHead><TableHead>Hours</TableHead><TableHead>DPR</TableHead><TableHead>DPR submitted</TableHead><TableHead>To-Dos</TableHead><TableHead>Resolved</TableHead></TableRow></TableHeader>
                  <TableBody>{dailyActivity?.rows?.map((r:any)=><TableRow key={r.employee.id}><TableCell><p className="font-medium">{r.employee.firstName} {r.employee.lastName}</p><p className="text-xs text-muted-foreground">{r.employee.employeeCode} · {r.employee.department?.name ?? '-'}</p></TableCell><TableCell>{r.attendance?.status ?? 'No record'}</TableCell><TableCell>{r.attendance?.checkInAt ? new Date(r.attendance.checkInAt).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'}) : '-'}</TableCell><TableCell>{r.attendance?.checkOutAt ? new Date(r.attendance.checkOutAt).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'}) : '-'}</TableCell><TableCell>{r.attendance?.workingHours != null ? Number(r.attendance.workingHours).toFixed(2) : '-'}</TableCell><TableCell>{r.dpr?.status ?? 'NOT SUBMITTED'}</TableCell><TableCell>{r.dpr?.submittedAt ? new Date(r.dpr.submittedAt).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'}) : '-'}</TableCell><TableCell>{r.todoSummary.total}</TableCell><TableCell>{r.todoSummary.resolved}</TableCell></TableRow>)}</TableBody>
                </Table>
              </div>
            </CardContent>
          </Card>
