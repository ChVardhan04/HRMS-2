...
import { CalendarDays, FileText, Sparkles, Laptop } from 'lucide-react';
...
export function EmployeeDashboard() {
  ...
  const { data: workDay } = useQuery({ queryKey: ['work-day', 'today'], queryFn: () => api.get<any>('/work-days/today'), refetchInterval: 60000 });
  return <div className="space-y-4">
    ...
    <Card><CardContent className="flex items-center justify-between gap-4 p-4"><div className="flex items-center gap-3"><Laptop className="h-5 w-5 text-primary" /><div><p className="text-sm font-medium">HRMS portal active time today</p><p className="text-xs text-muted-foreground">Keep HRMS open after check-in. The configured lunch window is excluded automatically.</p></div></div><div className="text-right"><p className="text-2xl font-semibold">{workDay?.portalActiveMinutes != null ? `${(Number(workDay.portalActiveMinutes) / 60).toFixed(1)}h` : '0.0h'}</p><p className="text-xs text-muted-foreground">{workDay?.portalHeartbeatCount ?? 0} hourly checks</p></div></CardContent></Card>
